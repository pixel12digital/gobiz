...
<head>
    ...
    <title>{{ __('Domain Rejected Notification') }}</title>
    <style>
        ...
        .title {
            font-size: 1.25rem;
            font-weight: 600;
            color: #e53e3e; /* Red color for rejected status */
            margin-bottom: 16px;
        }
        ...
        .button {
            ...
            padding: 12px 24px;
            background-color: #e53e3e; /* Red color for rejected action */
            color: #ffffff;
...
<body>
    <div class="container">
        <h2 class="title">{{ __('Domain Rejected Notification') }}</h2>
        <p class="text">{{ __('Hello ' . $customDomainRequestDetails['cardName'] . ',') }}</p>
        <p class="text">
            {{ __('We regret to inform you that your domain request has been rejected.') }}
        </p>
        <p class="text">
            {{ __('Here are the details of the rejected domain:') }}
        </p>
        ...
        <p class="text">{{ __('If you have any questions or need further assistance, please contact our support team.') }}</p>
