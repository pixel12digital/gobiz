            ...
            <div class="modal-footer">
                <div class="w-100">
                    <div class="row">
                        ...
                        <div class="col">
                            <a class="btn btn-danger w-100" id="payment_gateway_id">
                                {{ __('Yes, proceed') }}
                            </a>
                        </div>
                    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#payment-methods-table').DataTable({
            ...
            preDrawCallback: function(settings) {
                ...
                $('#payment-methods-table_wrapper').addClass('placeholder-glow');

                // Check if there are rows in the tbody after draw
                if ($('#payment-methods-table tbody tr').length === 0) {
                    // If there are no rows, add 10 placeholder rows with 6 columns each
                    var placeholderRows = '';
                    for (var i = 0; i < 10; i++) {
                        placeholderRows += '<tr>' + '<td class="text-center"><div class="placeholder placeholder-xs col-12"></div></td>'.repeat(6) + '</tr>';
                    }
                    $('#payment-methods-table tbody').html(placeholderRows);
                }
            },
            drawCallback: function(settings) {
                // Remove the placeholder-glow class once the table is fully rendered
                $('#payment-methods-table_wrapper').removeClass('placeholder-glow');

                // clear any existing placeholder rows
                $('#payment-methods-table tbody tr').each(function() {
                    var actionCell = $(this).find('td').eq(5); // Targeting the 5th column (index 5)
    ...
    function getPaymentMethod(paymentGatewayId, status) {
        "use strict";
        ...
        let message = `{{ __('If you proceed, this payment method will be :status.', ['status' => '${messageStatus}']) }}`;
        modalMessage.innerHTML = message.replace(':status', status);
        // Modal link
        link.getAttribute("href");
        link.setAttribute("href", "/admin/delete-payment-method?id=" + paymentGatewayId + '&status=' + status);
    }
</script>
@endsection
@endsection