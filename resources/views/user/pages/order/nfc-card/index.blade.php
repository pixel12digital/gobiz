        ...
        <div class="page-body">
            <div class="container-fluid">
                ...
                <div class="row row-cards">
                    {{-- Check NFC card designs in below 0 --}}
                    @if (count($availableNfcCards) > 0)
                        {{-- Nfc cards --}}
                        @foreach ($availableNfcCards as $nfcCard)
                            <div class="col-sm-12 col-lg-3 col-xl-3">
                                <div class="card">
                                    <!-- Photo -->
                                    <a href="{{ asset($nfcCard->nfc_card_front_image) }}" data-fslightbox
                                        data-fslightbox-gallery="gallery">
                                        <img src="{{ asset($nfcCard->nfc_card_front_image) }}"
                                            alt="{{ __($nfcCard->nfc_card_name) }}" class="card-img-top">
                                    </a>
                                    <a href="{{ asset($nfcCard->nfc_card_back_image) }}" data-fslightbox
                                        data-fslightbox-gallery="gallery">
                                        <img src="{{ asset($nfcCard->nfc_card_back_image) }}"
                                            alt="{{ __($nfcCard->nfc_card_name) }}" class="card-img-bottom d-none">
                                    </a>
                                    <div class="card-body">
                                        <h3 class="card-title">{{ __($nfcCard->nfc_card_name) }}</h3>
                                        <p class="text-secondary description">
                                            {{ __($nfcCard->nfc_card_description) }}
                                        </p>
                                        <div class="d-flex align-items-end mt-3">
                                            <p class="h3 me-2">{{ __('Price:') }}</p> <p class="h4 text-muted">{{ $currency->symbol }}{{ formatCurrency($nfcCard->nfc_card_price) }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="empty">
                                        <div class="empty-img"><svg class="img" xmlns="http://www.w3.org/2000/svg"
                                                ...
                                                <path
                                                    ...
                                                    fill="black" opacity="0.5" />
                                                <path
                                                    d="M501.775 256.991C497.672 256.079 492.049 254.407 486.122 251.368C480.802 248.632 476.851 245.593 473.812 243.009C476.243 242.705 483.234 242.249 490.529 246.049C497.216 249.544 500.559 254.711 501.775 256.991Z"
                                                    fill="#454C5E" />
                                            </svg></div>
                                        <p class="empty-title">{{ __('NFC cards Not Found!') }}</p>
