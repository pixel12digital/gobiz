...
<body dir="{{ App::isLocale('ar') || App::isLocale('ur') || App::isLocale('he') ? 'rtl' : 'ltr' }}">
        ...
        <div class="page-wrapper mt-5">
            <div class="page-body">
                <div class="container-xl">
                    ...
                    <div class="row">
                        {{-- Greeting message --}}
                        <div class="col-md-12 pt-6 px-1 pt-sm-12">
                                    ...
                                    @foreach ($products as $product)
                                        <div class="col-sm-6 col-lg-3 col-6 py-2">
                                            <div class="card custom-card-style">
                                                ...
                                                <div class="card-body">
                                                    <div class="badge-container d-flex flex-wrap align-items-center gap-2">
                                                        <span class="badge bg-yellow text-white" id="{{ $product->product_id }}_subtitle">
                                                    ...
                                                    <span class="d-none" id="{{ $product->product_id }}_price">{{ $product->sales_price }}</span>
                                                    <p class="pricing my-2 p-0"><strong><span
                                                                id="{{ $product->product_id }}_currency">{{ $currency }}
                                                                <span>{{ formatCurrency($product->sales_price) }}</span></span></strong>
                                                    </p>
                                                    @if ($product->sales_price != $product->regular_price)
                                                        <p class="regular_price"><del
                                                                class="text-yellow">{{ $currency }}
                                                                {{ formatCurrency($product->regular_price) }}</del></strong></p>
                                                    @endif
                                                </div>
    ...
    <script>
        function jsFormatCurrency(amount, decimalPlaces, formatType) {
            let formattedAmount;
            
            switch (formatType) {
                case "1,234,567.89": // '1,234,567.89' (US style)
                    formattedAmount = amount.toLocaleString('en-US', {
                        minimumFractionDigits: decimalPlaces,
                        maximumFractionDigits: decimalPlaces
                    });
                    break;
                case "1.234.567,89": // '1.234.567,89' (German style)
                    formattedAmount = amount.toLocaleString('de-DE', {
                        minimumFractionDigits: decimalPlaces,
                        maximumFractionDigits: decimalPlaces
                    });
                    break;
                case "1 234 567,89": // '1 234 567,89' (French style)
                    formattedAmount = amount.toLocaleString('fr-FR', {
                        minimumFractionDigits: decimalPlaces,
                        maximumFractionDigits: decimalPlaces
                    });
                    break;
                case "1'234'567.89": // "1'234'567.89" (Swiss style)
                    formattedAmount = amount.toFixed(decimalPlaces).replace(/\B(?=(\d{3})+(?!\d))/g, "'");
                    break;
                default:
                    // Default to '1,234,567.89' if an unknown formatType is passed (US style)
                    formattedAmount = amount.toLocaleString('en-US', {
                        minimumFractionDigits: decimalPlaces,
                        maximumFractionDigits: decimalPlaces
                    });
    ...
    <script>
        ...
        function updateList() {
            ...
            cart.forEach((item, index) => {
                ...
                var setDecimalsPlaces = {{ $config[56]->config_value ?? 2 ?? 2 }};
                ...
                cart_items +=
                    `<p class="text-dark mt-1"><strong>${currency} ${jsFormatCurrency(total_price, setDecimalsPlaces, formatType)}</strong></p>`;
                cart_items += `</div>`;
            ...
            if (grandTotal > 0) {
                ...
                cart_items +=
                    `<br><h3 class="pl-4 pt-4 pr-4 font-bold">{{ __('Grand total:') }} ${currency} ${jsFormatCurrency(grandTotal, setDecimalsPlaces, formatType)}</h3>`;
            }
        ...
        function createWhatsAppLink(cusDetails) {
            ...
            if (cusDetails[0].length >= 3 && cusDetails[1].length >= 4) {
                ...
                cart.forEach(item => {
                    ...
                    productsList +=
                        `${item.product_name} - ${item.qty} X ${currency} ${jsFormatCurrency(cartPrice, setDecimalsPlaces, formatType)} = *${currency} ${jsFormatCurrency(itemCost, setDecimalsPlaces, formatType)}*\n`;
                    grandTotal += itemCost;
                });

                // Add total and customer details to products list
                productsList += `\n- - - - - - - - - - - - - - - - - - - -\n`;
                productsList += `*{{ __('Total') }}* : *${currency} ${jsFormatCurrency(grandTotal, setDecimalsPlaces, formatType)}*\n\n`;
