...
<head>
    ...
    <!-- Tailwind CSS -->
    <link rel="stylesheet" href="{{ url('templates/css/restaurant.css') }}">
    {{-- Swiper CSS --}}
...
<body class="bg-red-50 min-h-screen"
    ...
    <div class="container max-w-2xl mx-auto lg:py-10 pb-20">
        ...
        @if ($business_card_details->password == null || Session::get('password_protected') == true)
                ...
                <div class="bg-white lg:rounded-2xl shadow-[0_0_4px_rgba(0,0,0,0.1)] overflow-hidden relative">
                    ...
                    @if ($business_card_details->cover_type == 'none')
                        <div class="lg:h-80 h-60 relative" id="profile">
                            {{-- Cover Image --}}
                            <img src="{{ url('img/templates/restaurant/banner.png') }}"
                    ...
                    <!-- Profile Info -->
                    <div class="px-6 -mt-0.5 pb-6 relative">
                        <img src="{{ url('img/templates/restaurant/bg.png') }}" alt=""
                            class="w-full object-cover absolute -mx-8 -top-[30px] z-0 opacity-60" />
                        <!-- Start Profile Info -->

                        {{-- Profile Image --}}
                        <div class="relative z-20 bg-transparent mt-8 flex">
                        ...
                        @if (count($feature_details) > 0)
                            <div class="flex flex-wrap gap-4 justify-center lg:ml-10 mt-6">
                                    ...
                                    @if (in_array($feature->type, ['tel', 'email', 'instagram', 'facebook']))
                                        ...
                                        @if ($feature->type == 'tel')
                                            <a href="tel:{{ $feature->content }}"
                                                class="bg-red-50 w-14 h-14 flex items-center justify-center rounded-full text-[#ed2031] hover:bg-red-100 transition-colors border border-[#ed2031] z-10">
                        ...
                        @if (count($feature_details) > 0)
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 relative mt-8">
                                <img src="{{ url('img/templates/restaurant/1.png') }}" alt=""
                                    class="w-32 absolute -top-14 -right-7 z-20 animate-move-y" />
                                @foreach ($feature_details as $feature)
                                    @if (in_array($feature->type, ['address']))
                                        <div class="transition-colors rounded-2xl w-full bg-cover bg-center border border-red-700"
                                            style="background-image: url('{{ url('img/templates/restaurant/card-bg.png') }}')">
                                            <!-- Font Awesome Icon -->
                                            <a href="https://www.google.com/maps/place/{{ urlencode($feature->content) }}"
                                                target="_blank" class="px-6 py-4 text-white flex flex-col">
                                                <div
                                                    class="p-3 border border-white rounded-full w-12 h-12 flex justify-center items-center">
                                                    <!-- Font Awesome Icon -->
                                                    <i class="{{ $feature->icon }} fa-xl"></i>
                                                </div>
                                                <!-- Title -->
                                                <h2 class="text-base font-semibold mt-1.5">{{ $feature->label }}</h2>
                                                <!-- Description -->
                        ...
                        @if (!empty($feature_details) && count($feature_details) > 0)
                            <div class="relative">
                                @php
                                    ...
                                    // Check if there are valid links to display
                                    $hasValidLinks = collect($feature_details)->contains(function ($feature) use ($excludedTypes) {
                                ...
                                @if ($hasValidLinks)
                                    <h2
                                        class="text-3xl lg:text-4xl font-bold text-gray-800 py-12 text-center relative head">
                                        <div class="absolute bottom-10 left-1/2 h-0.5 w-14 bg-[#ed2031] -mx-6 "></div>
                                        {{ __('Social Links') }}
                                    </h2>
                                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                        @foreach ($feature_details as $feature)
                                            ...
                                            @endphp
                                            @if (isset($feature->type) && !in_array($feature->type, $excludedTypes))
                                                <a href="{{ $href }}" target="_blank"
                                                    class="p-4 transition-colors rounded-2xl flex flex-col bg-cover bg-center border border-red-700"
                                                    style="background-image: url('{{ url('img/templates/restaurant/card-bg.png') }}');">
                                                    <!-- Font Awesome Icon -->
                                                    <i class="{{ $feature->icon ?? '' }} text-white text-xl mb-1"></i>
                                                    <!-- Title -->
                                                    <h2 class="text-md font-semibold text-white">{{ $feature->label ?? 'N/A' }}</h2>
                                                    <!-- Description -->
                        ...
                        @if (count($service_details) > 0)
                            <div class="relative">
                                <img src="{{ url('img/templates/restaurant/2.png') }}" alt=""
                                    class="w-36 absolute -top-0 -left-[90px] z-20 animate-spin"
                                ...
                                <swiper-container class="mySwiper" autoplay-delay="2500"
                                    ...
                                    space-between="20" autoplay-disable-on-interaction="false" loop="true">
                                    {{-- All services --}}
                                    @foreach ($service_details as $service_detail)
                                        <swiper-slide class="p-1">
                                            <!-- Service -->
                                            <div
                                                class="flex flex-col justify-center bg-white p-4 rounded-2xl shadow-[0_0_4px_rgba(0,0,0,0.1)]">
                                                {{-- Image --}}
                                                <img class="w-full h-44 object-cover rounded-2xl mb-4"
                                                    src="{{ url($service_detail->service_image) }}"
                                                    alt="{{ $service_detail->service_name }}" />
                                                {{-- Name --}}
                                                <h2 class="text-[#1C160C] text-lg font-bold mb-1">
                                                    {{ $service_detail->service_name }}
                                                </h2>
                                                {{-- Description --}}
                                                <p class="text-gray-500 text-sm font-normal mb-3">
                                                        ...
                                                        @if ($enquiry_button != null)
                                                            @if ($whatsAppNumberExists == true && $whatsAppNumberExists == true && $service_detail->enable_enquiry == 'Enabled')
                                                                <a href="https://wa.me/{{ $enquiry_button }}?text={{ __('Hi, I am interested in your product/service:') }} {{ $service_detail->service_name }}. {{ __('Please provide more details.') }}"
                                                                    target="_blank"
                                                                    class="text-white w-full px-12 lg:w-auto bg-[#ed2031] rounded-lg text-base border border-red-700 font-medium py-1.5 transition-colors block text-center">
                                                                    {{ __('Enquire') }}
                                                                </a>
                                                            @endif
                                                        @endif
                        ...
                        @if (count($product_details) > 0)
                            <div class="relative">
                                ...
                                <swiper-container class="mySwiper" autoplay-delay="2500"
                                    ...
                                    space-between="20" autoplay-disable-on-interaction="false">
                                    {{-- All products --}}
                                    @foreach ($product_details as $product_detail)
                                        <swiper-slide class="p-1">
                                            <!-- Product -->
                                            <div
                                                class="flex flex-col justify-center bg-white p-4 rounded-2xl shadow-[0_0_4px_rgba(0,0,0,0.1)] relative">
                                                {{-- Badge --}}
                                                @if (!empty($product_detail->badge))
                                                    <p
                                                        class="absolute top-7 right-7 font-medium text-white bg-[#ed2031] px-4 py-1 rounded-lg">
                                                ...
                                                <div class="flex flex-col">
                                                    ...
                                                    <div class="flex-flex-col">
                                                        <div>
                                                            <h4 class="text-[#000000] text-base font-bold">
                                                                {{ __('Price:') }}
                                                                <span
                                                        ...
                                                        @if ($whatsAppNumberExists == true)
                                                            <div class="mt-2 w-full">
                                                                <a href="https://wa.me/{{ $enquiry_button }}?text={{ __('Hi, I am interested in your product:') }} {{ $product_detail->product_name }}. {{ __('Please provide more details.') }}"
                                                                    target="_blank"
                                                                    class="text-white w-full px-12 lg:w-auto bg-[#ed2031] text-base font-semibold border border-red-700 py-1.5 rounded-lg transition-colors block text-center">
                                                                    {{ __('Enquire') }}
                                                                </a>
                                                            </div>
                                                        @endif
                        ...
                        @if (count($galleries_details) > 0)
                            <div class="relative">
                                <img src="{{ url('img/templates/restaurant/3.png') }}" alt=""
                                    class="w-32 absolute top-1 -right-12 z-20 animate-move-y" />
                                <h2
                                    class="text-3xl lg:text-4xl font-bold text-gray-800 py-12 text-center relative head">
                                    <div class="absolute bottom-10 left-1/2 h-0.5 w-14 bg-[#ed2031] -mx-6 "></div>
                                    {{ __('Gallery') }}
                                    ...
                                    <swiper-container class="mySwiper" autoplay-delay="3000"
                                        ...
                                        @foreach ($galleries_details as $galleries_detail)
                                            <swiper-slide class="p-1">
                                                <!-- Gallery -->
                                                <div
                                                    class="flex flex-col items-center justify-center bg-white rounded-t-2xl px-4 pt-4 shadow-[0_0_4px_rgba(0,0,0,0.1)]">
                                                    <img class="w-full h-64 object-cover rounded-t-xl {{ $galleries_detail->caption ? '' : 'rounded-b-2xl' }}"
                                                        src="{{ url($galleries_detail->gallery_image) }}"
                                                        alt="{{ $galleries_detail->caption }}" />
                                                </div>
                                                {{-- Title --}}
                                                @if ($galleries_detail->caption)
                                                    <div class="text-center bg-red-100 p-3 rounded-b-2xl">
                                        ...
                                        @if ($feature->type == 'youtube')
                                            ...
                                            <div class="rounded-2xl overflow-hidden">
                                                ...
                                                @if ($feature->label != null)
                                                    <div class="px-5 py-3 bg-red-100">
                        ...
                        @if (count($testimonials) > 0)
                            <div class="relative">
                                <img src="{{ url('img/templates/restaurant/4.png') }}" alt=""
                                    class="w-36 absolute top-1 -left-20 z-20 animate-move-y" />
                                <h2
                                    class="text-3xl lg:text-4xl font-bold text-gray-800 py-12 text-center relative head">
                                    <div class="absolute bottom-10 left-1/2 h-0.5 w-14 bg-[#ed2031] -mx-6 "></div>
                                    {{ __('Client Reviews') }}
                                ...
                                <swiper-container
                                    class="mySwiper lg:mx-5 rounded-tr-[80px] rounded-bl-[80px] shadow-[0_0_4px_rgba(0,0,0,0.1)] bg-cover bg-center"
                                    autoplay-delay="2000" autoplay-disable-on-interaction="false" loop="true"
                                    style="background-image: url('{{ url('img/templates/restaurant/card-bg.png') }}')">
                                    {{-- Client Reviews --}}
                                    @foreach ($testimonials as $testimonial)
                                        <!-- Testimonial -->
                                        <swiper-slide class="p-1 bg-transparent">
                                            <div
                                                class="flex flex-col items-center justify-center p-6 lg:p-10 text-center">
                                                {{-- Image --}}
                                                <img class="w-20 h-20 rounded-full mb-2 object-cover"
                                                    src="{{ url($testimonial->reviewer_image) }}"
                                                    alt="{{ $testimonial->reviewer_name }}" />
                                                {{-- Review --}}
                                                <p class="text-white text-lg mb-4 italic">
                                                    "{{ $testimonial->review }}"
                                                </p>
                                    ...
                                    <div class="bg-white rounded-lg py-4">
                                            ...
                                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                                @foreach (['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] as $day)
                                                    <div class="flex items-center space-x-4">
                                                        <!-- Day Icon -->
                                                        <div
                                                            class="flex items-center justify-center w-10 h-10 bg-red-100 text-[#ed2031] rounded-full border border-[#ed2031]">
                                        ...
                                        @else
                                            ...
                                            <div class="flex items-start space-x-4">
                                                <!-- Animated Icon -->
                                                <div
                                                    class="flex items-center justify-center w-12 h-12 bg-red-100 text-[#ed2031] rounded-full transform hover:scale-110 transition-transform duration-300 ease-in-out">
                        ...
                        @if ($appointmentEnabled == true && isset($plan_details['appointment']) == 1)
                            <div class="relative">
                                <img src="{{ url('img/templates/restaurant/5.png') }}" alt=""
                                    class="w-40 absolute -top-3 -right-16 z-20 animate-move-y" />
                                ...
                                @if ($plan_details['appointment'] == 1)
                                    @if ($appointment_slots != null)
                                        ...
                                        <h2
                                            class="text-3xl lg:text-4xl font-bold text-gray-800 py-12 text-center relative head">
                                            <div class="absolute bottom-10 left-1/2 h-0.5 w-14 bg-[#ed2031] -mx-6 ">
                                            </div>
                                            {{ __('Appointment') }}
                                        </h2>

                                        <div
                                            class="shadow-[0_0_4px_rgba(0,0,0,0.1)] overflow-hidden border rounded-2xl p-6 bg-red-100 border border-red-300">
                                            ...
                                            <div
                                                ...
                                                <input type="text" id="appointment-date"
                                                    class="flatpickr-input md:w-1/2 w-full px-4 py-2.5 text-gray-800 bg-white border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#ed2031] focus:ring-opacity-50"
                                                    placeholder="{{ __('Select a date') }}" />
                                                <!-- Select time in dropdown -->
                                                <select id="time-slot-select"
                                                    class="md:w-1/2 w-full px-4 py-3 text-gray-800 bg-white border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#ed2031] focus:ring-opacity-50">
                                            ...
                                            <div class="flex justify-center">
                                                <button id="add-slot-button"
                                                    class="w-full p-2.5 bg-[#ed2031] text-white text-lg text-center rounded-xl font-semibold border border-red-700"
                            ...
                            <div class="relative">
                                ...
                                <div class="grid lg:grid-cols-2 gap-4">
                                    ...
                                    @foreach ($payment_details as $payment)
                                        ...
                                        <div class="flex flex-col border border-red-700 bg-[#ed2031] rounded-2xl p-4">
                        ...
                        @if ($plan_details['contact_form'] == 1)
                            @if ($business_card_details->enquiry_email != null)
                                <div class="relative">
                                    <img src="{{ url('img/templates/restaurant/6.png') }}" alt=""
                                        class="w-40 absolute -top-3 -left-20 z-20 animate-move-y" />
                                    <h2
                                        class="text-3xl lg:text-4xl font-bold text-gray-800 py-12 text-center relative head">
                                        <div class="absolute bottom-10 left-1/2 h-0.5 w-14 bg-[#ed2031] -mx-6 "></div>
                                        {{ __('Contact Us') }}
                                    ...
                                    @if (Session::has('message'))
                                        <div
                                            class="px-6 py-4 bg-red-50 border border-red-200 rounded-lg shadow-[0_0_4px_rgba(0,0,0,0.1)] mb-6">
                                    ...
                                    {{-- Contact Form --}}
                                    <form
                                        class="w-full max-w-full border p-6 rounded-2xl bg-red-50 border-red-300 relative shadow-[0_0_4px_rgba(0,0,0,0.1)]"
                                        ...
                                        <div class="grid grid-cols-1 gap-2 lg:gap-6 lg:grid-cols-2">
                                            ...
                                            <div class="grid grid-cols-1 gap-2">
                                                ...
                                                <div>
                                                    ...
                                                    <input type="text" name="name" placeholder="{{ __('Your Name') }}"
                                                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#ed2031] focus:ring-opacity-50"
                                                        required />
                                                </div>
                                                {{-- Email --}}
                                                <div>
                                            ...
                                            <div class="h-full pb-8">
                                                ...
                                                <textarea name="message" placeholder="{{ __('Your Message') }}"
                                                    class="w-full h-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#ed2031] focus:ring-opacity-50 resize-none"
                                        ...
                                        <div class="mt-6">
                                            <button type="submit"
                                                class="w-full px-4 py-3 bg-[#ed2031] text-white text-lg font-semibold rounded-xl focus:outline-none border border-red-700">
                                                {{ __('Send') }}
                                            </button>
                                        </div>
                                    </form>
                            ...
                            <div class="relative">
                                ...
                                @foreach ($feature_details as $feature)
                                    @if ($feature->type == 'map')
                                        ...
                                        {{-- Map title --}}
                                        @if ($feature->label != null)
                                            <div class="px-5 py-3 bg-red-100 rounded-b-2xl">
                                                <div class="mb-2">
                            ...
                            <div class="pb-1">
                                <div
                                    ...
                                    <div class="mt-2 text-gray-500">
                                        {{ __('Copyright') }} &copy;
                                        <a class="text-[#ed2031]" href="{{ url($card_details->card_url) }}">
                        ...
                        @else
                            <div class="pb-1">
                                <div
                                    ...
                                    <div class="mt-2 text-gray-500">
                                        {{ __('Made with') }}
                                        <a class="text-[#ed2031]" href="{{ env('APP_URL') }}">
                                            {{ config('app.name') }} </a>
            ...
            <div
                class="fixed w-11/12 left-1/2 bottom-4 bg-red-200/60 border border-red-500 rounded-full backdrop-blur-md py-4 px-3 flex lg:hidden md:hidden transform -translate-x-1/2 z-50">
                <!-- Profile Icon -->
                <div class="flex-1 flex items-center justify-center">
                    <a class="border border-[#ed2031] p-3 rounded-full bg-red-200" href="#profile">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round"
                            class="icon icon-tabler icons-tabler-outline icon-tabler-user text-[#ed2031] h-6 w-6">
                ...
                <!-- Send Icon -->
                <div class="flex-1 flex items-center justify-center">
                    <button class="border border-[#ed2031] p-3 rounded-full bg-red-200"
                        onclick="toggleWhatsAppModal(true)">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round"
                            class="icon icon-tabler icons-tabler-outline icon-tabler-send text-[#ed2031] h-6 w-6">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                            <path d="M10 14l11 -11" />
                ...
                <div class="flex-1 flex items-center justify-center">
                    <a href="{{ route('download.vCard', $business_card_details->card_id) }}"
                        class="border border-[#ed2031] p-3 rounded-full bg-red-200">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round"
                            class="icon icon-tabler icons-tabler-outline icon-tabler-download text-[#ed2031] h-6 w-6">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                ...
                <div class="flex-1 flex items-center justify-center">
                    <button class="border border-[#ed2031] p-3 rounded-full bg-red-200"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round"
                            class="icon icon-tabler icons-tabler-outline icon-tabler-line-scan text-[#ed2031] h-6 w-6">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                ...
                <div class="flex-1 flex items-center justify-center">
                    <button class="border border-[#ed2031] p-3 rounded-full bg-red-200"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round"
                            class="icon icon-tabler icons-tabler-outline icon-tabler-share text-[#ed2031] h-6 w-6">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
        ...
        <div id="appointmentModal"
            ...
            <!-- Modal Content -->
            <div class="bg-red-50 rounded-xl w-full max-w-md p-6 mx-4 shadow-lg">
                <!-- Modal Header -->
                ...
                <form id="appointmentForm">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="name"
                            class="mt-1 p-2 border border-gray-300 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-[#ed2031] focus:ring-opacity-50"
                            required>
                    </div>
                    ...
                    <div class="flex justify-between">
                        ...
                        <button type="submit" id="bookAppointmentButton"
                            class="bg-[#ed2031] text-white px-4 py-2 rounded-lg">
                            {{ __('Submit') }}
                        </button>
                    </div>
                </form>
        ...
        <div id="shareModal"
            ...
            <!-- Modal content -->
            <div class="bg-red-50 rounded-xl w-full max-w-md p-6 mx-4 space-y-6" onclick="event.stopPropagation()">
                <!-- Modal header -->
                ...
                <div class="flex justify-around text-[#ed2031]">
                ...
                <div class="flex justify-center">
                    <button onclick="copyLink()"
                        class="bg-[#ed2031] text-white font-bold py-2 px-4 rounded-xl w-full border border-red-700">
                        {{ __('Copy Link') }}
                    </button>
                </div>
            </div>
        ...
        <div id="whatsappModal"
            ...
            <div class="bg-red-50 border-red-200 rounded-2xl w-full max-w-md p-6 mx-4 space-y-6"
                ...
                <div>
                    ...
                    <input type="text" id="whatsappNumber" placeholder="{{ __('e.g., +919876543210') }}"
                        class="w-full mt-1 px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#ed2031] focus:ring-opacity-50" />
                </div>
        ...
        <div id="scanModal"
            ...
            <!-- Modal content (stops propagation to prevent closing when clicking inside) -->
            <div class="rounded-2xl w-full max-w-md p-6 mx-4 space-y-6 border-red-100 bg-red-50 qr-modal-overlay"
                onclick="event.stopPropagation()">
                <!-- Qr Code -->
                ...
                <div class="flex justify-center">
                    <button id="download"
                        ...
                        class="bg-red-100 border border-red-200 font-bold p-3 rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round"
                            class="icon icon-tabler icons-tabler-outline icon-tabler-download text-[#ed2031] h-6 w-6">
                        ...
                        <div class="bg-white rounded-lg p-6 w-96 max-w-full shadow-lg transform transition-all duration-300"
                            ...
                            <div class="mt-6 space-y-4">
                                <form action="{{ route('check.pwd', $business_card_details->card_id) }}"
                                    ...
                                    {{-- Message --}}
                                    @if (Session::has('message'))
                                        <div class="flex items-center p-4 my-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-green-800 dark:text-red-400"
                                            role="alert">
